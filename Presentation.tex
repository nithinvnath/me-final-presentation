...
\begin{frame}{Adversarial Model}
	\begin{itemize}
        ...
        \item \textsc{State} procedure returns the server's state - including stored ciphertexts, public parameters, etc. Only read only access is gained using this.
    \end{itemize}
\end{frame}
...
\begin{frame}{DD-Across Recovery}
	\begin{itemize}
		...
		\item For $\adv$ to win, a mismatch in the plaintext $m$ put on the server and the plaintext $m'$ recovered using $\mathsf{Get}$.\\
		\item Immutability of the tables means once put, it cannot be changed.
		\item Only possibility is hash collision.
	\end{itemize}
\end{frame}
